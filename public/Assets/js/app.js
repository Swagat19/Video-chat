var AppProcess = (function(){
    ...
    async function eventProcess(){
        $("#micMuteUnmute").on("click",async function(){
            ...
            if(isAudioMute){
                ...
                udpateMediaSenders(audio,rtp_aud_senders);
            }else {
                audio.enabled = false;
    ...
    async function udpateMediaSenders(track,rtp_senders){
    ...
    async function videoProcess(newVideoState){
        ...
        try{
            ...
            if(vstream && vstream.getVideoTracks().length > 0){
                ...
                if(videoCamTrack){
                    ...
                    udpateMediaSenders(videoCamTrack,rtp_vid_senders)
                }
            }
        }catch(e){
            console.log(e);
        ...
        video_st = newVideoState;
        if(newVideoState == video_states.Camera){
            $("#videoCamOnOff").html('<span class="material-icons" style="width: 100%;">videocam</span0>');
            $("#ScreenShareOnOf").html('<span class="material-icons ">present_to_all</span><div> Present Now</div>');
        }else if(newVideoState == video_states.ScreenShare){
            ...
            $("#videoCamOnOff").html('<span class="material-icons" style="width: 100%;">videocam_off</span0>');
        }
    ...
    return {
        ...
        processClintFunc : async function(data,from_connid){
...
var MyApp = (function(){
    ...
    function init(uid,mid){
       ...
       event_proscess_for_signaling_server();
       eventHandeling();
       
    }
    function event_proscess_for_signaling_server(){
        socket = io.connect();
        ...
        socket.on("SDPProcess",async function(data){
            await AppProcess.processClintFunc(data.message , data.from_connid);
        });
